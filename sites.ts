...
export class CoreSitesProvider {
    ...
    /**
     * Gets a user token from the server using pfnumber of employee.
     *
     * @param {string} siteUrl The site url.
     * @param {string} pfnumber User name.
     * @param {string} pin Password.
     * @param {string} [service] Service to use. If not defined, it will be searched in memory.
     * @param {boolean} [retry] Whether we are retrying with a prefixed URL.
     * @return {Promise<CoreSiteUserTokenResponse>} A promise resolved when the token is retrieved.
     */
    getUserTokenFromPin(siteUrl: string, pfnumber: string, pin: string, otpmode?: boolean, service?: string, retry?: boolean)
        : Promise<CoreSiteUserTokenResponse> {
        if (!this.appProvider.isOnline()) {
            return Promise.reject(this.translate.instant('core.networkerrormsg'));
        }
        ...
        const params = { pfnumber: pfnumber, pin: pin, service: service};

        if (otpmode) {
            const params = { pfnumber: pfnumber, otp: pin, service: service};
        }

        const loginUrl = siteUrl + '/local/pin_authentication/token.php',
            promise = this.http.post(loginUrl, params).timeout(CoreConstants.WS_TIMEOUT).toPromise();
                ...
                if (typeof data.token != 'undefined') {
                    ...
                    if (typeof data.error != 'undefined') {
                        ...
                        if (!retry && data.errorcode == 'requirecorrectaccess') {
                            ...
                            if (otpmode) {
                                return this.getUserTokenFromPin(siteUrl, pfnumber, pin, true, service, true);
                            } else {
                                return this.getUserTokenFromPin(siteUrl, pfnumber, pin, false, service, true);
                            }
